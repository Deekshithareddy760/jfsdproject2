...
export default function ExpertLogin({ onExpertLogin }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:2032/checkexpertlogin', formData);
      if (response.data != null) 
      {
        ...
        navigate("/expert/expertDashboard");
      } 
      else 
      {
      ...
      <Grid container spacing={0}>
        ...
        <Grid item xs={12} md={6}>
          <StyledPaper elevation={3}>
            <StyledTitle>
              <div className="main">
                <h3 className="sign" align="center">Expert Login</h3>
